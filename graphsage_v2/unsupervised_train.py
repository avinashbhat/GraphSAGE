...
from graphsage_v2.models import SampleAndAggregate, SAGEInfo, Node2VecModel
from graphsage_v2.minibatch import EdgeMinibatchIterator
from graphsage_v2.neigh_samplers import UniformNeighborSampler
from graphsage_v2.utils import load_data
...
def train(train_data, test_data=None):
    ...
    if FLAGS.save_embeddings:
        ...
        if FLAGS.model == "n2v":
            ...
            train_ids = tf.constant([[id_map[str(n)]] for n in list(G.nodes()) if not G.nodes[n]['val'] and not G.nodes[n]['test']],
                    dtype=tf.int32)
            test_ids = tf.constant([[id_map[str(n)]] for n in list(G.nodes()) if G.nodes[n]['val'] or G.nodes[n]['test']], 
                    dtype=tf.int32)
            ...
            # run random walks
            from graphsage_v2.utils import run_random_walks
            nodes = [n for n in list(G.nodes()) if G.nodes[n]["val"] or G.nodes[n]["test"]]
